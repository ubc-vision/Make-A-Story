...
def calculate_activation_statistics(imgs, model, batch_size=32, dims=2048, cuda=False, normalize=False, verbose=0, is_ref=False):
    ...
    batch_size = 1
    im_input_size = 299
    transform_vlc = transforms.Compose([transforms.Resize((299, 299)),transforms.ToTensor(),transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    gt_images = np.load("/ubc/cs/research/shield/projects/trahman8/snap_research/VLCStoryGAN/VLCStoryGan/output/flintstones_martt_bs12_im60_2_stageI_r1.0/Test/gt_images-epoch-60.npy")
    with torch.no_grad():
        features = []
        features_cache = []
        ...
        #for i, (images, labels, bg_labels, text_decs) in tqdm(enumerate(dataloader)):
        for i, inputs in tqdm(enumerate(gt_images)):
            inputs = gt_images[i] #images[0]
            
            inputs = torch.clamp((torch.Tensor(inputs)+1.0)/2.0, min=0.0, max=1.0)
            inputs = (255. * rearrange(inputs.cpu().numpy(), 'c b h w -> b h w c')).astype(np.uint8)[:4]
            
            #inputs = (((rearrange(inputs, 'c b h w -> b c h w ')+ 1)/2) * 255.).astype(np.uint8)[:4]
            #images = inputs 
            num_frame = inputs.shape[0]
            images = torch.zeros(num_frame, 3, im_input_size, im_input_size)
            for k in range(num_frame):
                images[k] =  transform_vlc(Image.fromarray(inputs[k]))

        
            #video_len, n_channels, h, w = images.shape[1], images.shape[0], images.shape[2], images.shape[3]
...
def calculate_activation_statistics_vlc(imgs, model, batch_size=32, dims=2048, cuda=False, normalize=False, verbose=0, is_ref=False):
    if cuda:
        device = torch.device('cuda')
    else:
        device = torch.device('cpu')
    model.to(device)
    model.eval()
    transform_vlc = transforms.Compose([transforms.Resize((299, 299)),transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]) #transforms.Compose([transforms.Resize((299, 299)),transforms.ToTensor(),transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    given_images = np.load("/ubc/cs/research/shield/projects/trahman8/snap_research/VLCStoryGAN/VLCStoryGan/output/flintstones_martt_bs12_im60_2_stageI_r1.0/Test/images-epoch-60.npy")
    im_input_size = 299
    with torch.no_grad():
        features = []
        features_cache = []
        ...
        if verbose > 0:
            ...
            iter_dataset = dataloader
        #for i, (_, labels, bg_labels, text_decs) in tqdm(enumerate(dataloader)):
        for i, inputs in tqdm(enumerate(given_images)):
            inputs = given_images[i] #images[0]
            inputs = torch.clamp((torch.Tensor(inputs)+1.0)/2.0, min=0.0, max=1.0)
            inputs = (255. * rearrange(inputs.cpu().numpy(), 'c b h w -> b h w c')).astype(np.uint8)[:4]
            #inputs = (((rearrange(inputs, 'c b h w -> b c h w ')+ 1)/2) * 255.).astype(np.uint8)[:4]
            #images = inputs
...
def calculate_activation_statistics_ldm_model(imgs, model, model_path, batch_size=32, dims=2048,
	...
	config = OmegaConf.load("configs/latent-diffusion/flintstones_txt2img-1p4B-eval.yaml")
...
def fid_score(r_imgs, model_path, batch_size=32, dims=2048, cuda=False,
    ...
    # ground truth 
    m2, s2, f2 = calculate_activation_statistics_vlc(r_imgs, model, batch_size, dims, cuda, normalize)
