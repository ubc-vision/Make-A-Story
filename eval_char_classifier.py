...
def evaluate_gt(image_path, model_name, model_path, mode):
    ...
    total_char_positives = np.zeros(3)
    
    # Iterate over data.
    for i, (inputs, _,  labels, _, text_decs) in tqdm(enumerate(dataloader)):
    ...
    char_class = ['Mugen', 'LadyMugen', 'BabyMugen']
...
def evaluate_autoencoder(image_path, model_name, model_path, mode):
    ...
    model = load_model_from_config(config, "logs/2022-09-15T02-30-17_mugen_kl_8x8x16/checkpoints/epoch=000029.ckpt")
    ...
    # Iterate over data.
    for i, ( _, inputs, labels, _, text_decs) in tqdm(enumerate(dataloader)):
        ...
        with torch.no_grad():
            encoder_posterior = model.encode_first_stage(inputs[:,:,0])
            z = model.get_first_stage_encoding(encoder_posterior).detach()
            xrec = model.decode_first_stage(z)
            xrec = torch.clamp((xrec+1.0)/2.0, min=0.0, max=1.0).unsqueeze(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='models/inception_32_1e-05/epoch-23.pt')
